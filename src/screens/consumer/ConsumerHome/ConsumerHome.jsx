...
const ConsumerHome = ({ onNavigate, onHelpClick, onAddToCart }) => {
    ...
    const products = useMemo(() => Array.from({ length: 20 }).map((_, i) => {
        const baseProducts = [
    ...
    return (
        <div className="min-h-screen bg-[#1B1109] flex flex-col pb-24 relative overflow-x-hidden">
            ...
            <div className="px-6 mb-8">
                <div className="w-full aspect-[2/1] lg:aspect-[3/1] rounded-[40px] overflow-hidden relative shadow-2xl group">
                    <img
                        src={bannerImg}
                        alt="Mega Sale"
            ...
            {/* Product Grid */}
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 px-6 mb-8">
                {products.map(prod => (
                    <ProductCard
                        key={prod.id}
                        {...prod}
