...
const StatCard = ({ title, value, color, icon: Icon, onClick }) => (
    <div
        onClick={onClick}
        className={`w-full sm:min-w-[160px] h-[220px] ${color} rounded-[40px] p-6 flex flex-col justify-between shadow-lg cursor-pointer active:scale-95 transition-transform`}>
...
const ConsumerProfileScreen = ({ onBack, onNavigate }) => {
    return (
        <div className="min-h-screen bg-[#1B1109] flex flex-col relative overflow-hidden pb-32">
            {/* Header / Background */}
            <div className="h-64 sm:h-80 w-full relative overflow-hidden md:rounded-[60px] shadow-2xl max-w-6xl mx-auto md:mt-6">
                <img
                    src={profileBg}
                    alt="Roses"
                    className="w-full h-full object-cover"
                />
                <div className="absolute inset-0 bg-gradient-to-b from-black/20 to-[#1B1109]/80 md:to-[#1B1109]/40"></div>

                {/* Header Buttons */}
                <div className="absolute top-8 sm:top-12 left-6 right-6 flex justify-between items-center">
                    <button
                        onClick={onBack}
            ...
            {/* Stats Section */}
            <div className="mt-10 max-w-6xl mx-auto w-full px-6 mb-8">
            ...
            {/* Bottom Navigation */}
            <div className="fixed bottom-8 left-1/2 -translate-x-1/2 w-[90%] max-w-[500px] bg-[#A8B77D] rounded-[40px] px-8 py-4 flex justify-between items-center shadow-2xl z-50">
            ...
            {/* Logout Button */}
            <div className="px-6 mt-8 mb-32 max-w-md mx-auto w-full">
                <button
