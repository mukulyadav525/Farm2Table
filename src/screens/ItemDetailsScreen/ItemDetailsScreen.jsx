...
const ItemDetailsScreen = ({ item, onBack, onAddToCart }) => {
    ...
    return (
        <div className="min-h-screen bg-[#1B1109] flex flex-col relative text-white">
            {/* Header / Image Area */}
            <div className="relative w-full aspect-[4/3] rounded-b-[40px] overflow-hidden shadow-2xl">
                <img
                    src={item.image}
                    alt={item.title}
                    className="w-full h-full object-cover"
                />
                <div className="absolute inset-x-0 top-0 pt-12 px-6 flex justify-between items-start z-10">
                    <button
                        onClick={onBack}
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-[#1B1109] via-transparent to-transparent opacity-80"></div>
            </div>
            ...
            <div className="flex-1 px-6 -mt-8 relative z-20">
                ...
                {/* Price & Cart Actions */}
                <div className="bg-[#2D1B0F] rounded-[32px] p-6 border border-white/5 mt-auto mb-8 shadow-xl">
            ...
            {showToast && (
                <Toast
                    ...
                    onClose={() => { }} // No-op since we auto-navigate
                />
            )}
        </div>
    );
