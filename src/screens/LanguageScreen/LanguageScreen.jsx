...
const LanguageOption = ({ label, isSelected, onClick }) => {
    return (
        <button
            onClick={onClick}
            className={`w-full p-5 rounded-3xl flex items-center justify-between mb-3 transition-all duration-200 border border-white/5 ${isSelected
...
const LanguageScreen = ({ onBack, onContinue, onHelpClick }) => {
        ...
        <div className="min-h-screen bg-[#120B05] flex flex-col px-6 pt-safe-top">
            ...
            <div className="flex-1 overflow-y-auto">
                ...
                <div className="mt-2 text-right">
                    <button className="text-[#889E58] font-medium text-sm hover:text-[#9ebb64] transition-colors">
                        See All
                    </button>
                </div>
            </div>
